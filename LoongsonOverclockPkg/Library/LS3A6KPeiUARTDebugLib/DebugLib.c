...
VOID LS3A6KUART0WriteString(CHAR8 *String) {
    while(*String) {
        while (!(AsmIOCSRRead8(UART_BASE+UART_LSR) & (UART_LSR_THRE)));
        AsmIOCSRWrite8(UART_BASE+UART_TX, *String);
        String++;
    }
}
  ...
  )
{
  ...
  LS3A6KUART0WriteString (AsciiBuffer);
}
